...
/**
 * @brief Internal Alarm Clock for TraCIScenarioManager
 * * This function catches self-scheduled messages (timers) set by OMNeT++.
 * It acts as the event dispatcher for the simulation's lifecycle.
 * * Key Triggers:
 * 1. connectAndStartTrigger: Fires at the exact simulation start time (e.g., t=1.0s). 
 * -> GRPC THESIS NOTE: The gRPC Channel is initialized here rather than in 
 * initialize() to guarantee that SUMO is fully booted up and ready to 
 * accept connections at the exact synchronized start time.
 * * 2. executeOneTimestepTrigger: The continuous heartbeat loop.
 * -> GRPC THESIS NOTE: This triggers our custom gRPC executeOneTimestep() 
 * function to pull the BulkStateResponse from SUMO.
 */
void TraCIScenarioManager::handleSelfMsg(cMessage* msg)
{
    if (msg == connectAndStartTrigger) {
        ...
        if (grpcLogFile.is_open()) {
            grpcLogFile << "Time,VehicleID,OMNeT_X,OMNeT_Y,OMNeT_Z,Speed,Acceleration,Length,Width,Height,Signals,Type\n";
...
void TraCIScenarioManager::executeOneTimestep()
{
    ...
    if (!stub_) {
        std::cout << "\n=======================================================" << std::endl;
        std::cout << "  GRPC CLIENT WAKING UP! CONNECTING TO SERVER... " << std::endl;
        std::cout << "=======================================================\n" << std::endl;
        std::shared_ptr<grpc::Channel> channel = grpc::CreateChannel("localhost:50051", grpc::InsecureChannelCredentials());
        stub_ = veinsthesis::SumoCosimulation::NewStub(channel);
    }
    // ----------------------------------------------
    EV_DEBUG << "Triggering gRPC server simulation advance to t=" << simTime() << endl;
